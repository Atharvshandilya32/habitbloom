...
export async function searchUsers(
  ...
  if (!database || !queryStr.trim()) return [];

  const rawQuery = queryStr.trim().toLowerCase();
  const rawQueryHb = queryStr.trim().replace(/-/g, "").toLowerCase();
  const results: UserSocialProfile[] = [];
  const MAX_RESULTS = 15;
  const seenUids = new Set<string>();

  const addResult = (prof: UserSocialProfile) => {
    if (prof.uid !== currentUid && !seenUids.has(prof.uid)) {
      results.push(prof);
  ...
  try {
    ...
    const hbSnap = await get(ref(database, `hbIds/${rawQueryHb}`));
    if (hbSnap.exists()) {
      ...
      if (matchedUid !== currentUid) {
        ...
        if (profSnap.exists()) {
          addResult(profSnap.val() as UserSocialProfile);
        }
      }
    }

    // 2. Server-side Indexed Prefix Queries (replaces O(N) full table scan)
    if (results.length < MAX_RESULTS) {
      ...
      if (usernameSnap.exists()) {
        usernameSnap.forEach((childSnap) => {
          addResult(childSnap.val() as UserSocialProfile);
        });
      }
    }

    if (results.length < MAX_RESULTS) {
      // Query by displayName (Note: this is case-sensitive on Firebase side, but typically displayNames start capitalized)
      // Since RTDB lacks robust case-insensitive searching, we query lowercase prefix or exact casing
      // We can do a basic lowercase prefix search if displayNames are lowercase, but they usually aren't.
      // To improve, we can query both capitalized and lowercase prefix.

      // We'll search by capitalized prefix and lowercase prefix to capture common patterns
      const capitalizedQueryStr =
      ...
      const queries = [
        ...
        query(
          ref(database, "socialProfiles"),
          orderByChild("displayName"),
          startAt(capitalizedQueryStr),
          endAt(capitalizedQueryStr + "\uf8ff"),
          limitToFirst(MAX_RESULTS),
        ),
      ];
      ...
      for (const q of queries) {
        ...
        if (snap.exists()) {
          snap.forEach((childSnap) => {
            const prof = childSnap.val() as UserSocialProfile;
            // Additional client-side verification for case-insensitivity on substring match just in case
            if (prof.displayName.toLowerCase().includes(rawQuery)) {
              addResult(prof);
            }
          });
        }
