...
export default function WeeklyGoals({ habits, habitLogs }: WeeklyGoalsProps) {
  ...
  const weeklyGoals = goals.filter((g) => g.type === 'weekly' && isGoalActive(g));
  ...
  const handleAddGoal = () => {
    ...
    if (habit) {
      const newGoal = createGoal(habit.id, habit.name, 'weekly', parseInt(target), unit);
      addGoal(newGoal);
      setSelectedHabit('');
      setTarget('');
      setUnit('times');
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="grid gap-4">
        {weeklyGoals.length === 0 ? (
          <p className="text-gray-500 text-center py-8">No active weekly goals. Create one to get started!</p>
        ) : (
